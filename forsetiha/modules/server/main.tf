...
data "template_file" "forseti_server_config" {
  ...
  vars {
    ...
    CSCC_SOURCE_ID          = "${google_scc_source.forseti_source.name}"
...
#---------------------------#
# Create Forseti SCC source #
#---------------------------#

resource "google_scc_source" "forseti_source" {
  display_name = "Forseti Findings"
  organization = "${var.org_id}"
  description  = "Forseti Findings for security command center"
}
...
// This rule allows health checks to reach forseti instances
resource "google_compute_firewall" "forseti-server-healthcheck-allow" {
  name                    = "forseti-server-healthcheck-allow-${local.random_hash}"
  project                 = "${local.network_project}"
  network                 = "${var.network}"
  target_service_accounts = ["${google_service_account.forseti_server.email}"]
...
// This rules enable instances without external IPs to contact GCP APIs through private access

resource "google_compute_firewall" "forseti-server-private-access" {
  name                    = "forseti-server-private-access-${local.random_hash}"
  project                 = "${local.network_project}"
  network                 = "${var.network}"
  target_service_accounts = ["${google_service_account.forseti_server.email}"]
  direction               = "EGRESS"
...
//This adds the failover DB
resource "google_sql_database_instance" "failover" {
  name                 = "${local.cloudsql_name}-failover"
  project              = "${var.project_id}"
  region               = "${var.cloudsql_region}"
  database_version     = "MYSQL_5_7"
...
resource "google_sql_database_instance" "read_replica" {
  name                 = "${local.cloudsql_name}-read-replica"
  project              = "${var.project_id}"
  region               = "${var.cloudsql_region}"
  database_version     = "MYSQL_5_7"
  master_instance_name = "${google_sql_database_instance.master.name}"
